...
public class STabbedPane extends JTabbedPane {  
    ...
    class STabbedPaneUI extends BasicTabbedPaneUI {  
        ...
        private void initialize() {  
        	//设置整个Panel的边界
            UIManager.put("TabbedPane.contentAreaColor", Color.decode("#EEEEEE"));  
                ...
                public void mousePressed(MouseEvent e) {  
                    for (int i = 0; i < getTabCount(); i++) {  
                        if (closeRects[i].contains(e.getPoint())  
                                && closable.get(i)) {  
                        	SerachPage.histroyPanel.remove(i);
                            removeTab(i);  
                            System.out.println(i);
                            for(int count = i;count< getTabCount();count++) {
                            	final int k = count;
                            	SerachPage.serachLabel[k+1].addMouseListener(new MouseAdapter() {
                            		public void mouseClicked(MouseEvent e) {
                            			SerachPage.mainTable.setSelectedIndex(k);
                            		}
                            	});
                            }
                        }  
            ...
            addMouseMotionListener(new MouseAdapter() {  
                ...
                public void mouseMoved(MouseEvent e) {  
                	...
                	/*for (int i = 0; i < getTabCount(); i++) {
                		if(rects[1].x<=e.getX()&&e.getX()<=rects[1].width&&rects[1].y<=e.getY()
                				&&e.getY()<=rects[1].height) {
                			System.out.println(tabRuns[i]);
                		}
                	}*/
                }  
            });  
        ...
        protected void paintTabBackground(Graphics g, int tabPlacement,  
            ...
            case LEFT:  
                if (isSelected) {  
                    gradient = new GradientPaint(x+1, y, colorNorth,   
                            x+w, y, colorNorth, true);  
                } else {  
                    gradient = new GradientPaint(x+1, y, colorSouth,   
            ...
            case RIGHT:  
                if (isSelected) {  
                    gradient = new GradientPaint(x+w, y, colorNorth,   
                            x+1, y, colorNorth , true);  
                } else {  
                    gradient = new GradientPaint(x+w, y, colorSouth,   
            ...
            case BOTTOM:  
                if (isSelected) {  
                    ...
                    gradient = new GradientPaint(x+1, y+h, colorSouth,   
                            x+1, y, colorSouth, true);  
                }  
                g2d.setPaint(gradient);  
        ...
        private void paintCloseIcon(Graphics g, int tabIndex, boolean entered) {  
            ...
            int[] ys = { y, y, y + 2, y + 2, y, y, y + 2, y + 4, y + 5, y + 7,  
                    ...
                    y + 4, y + 2 };  
            if (entered) {  
                g.setColor(Color.decode("#fe5864"));  
            } else {  
        ...
        class TabbedPaneLayout extends BasicTabbedPaneUI.TabbedPaneLayout {  
            ...
            protected void calculateTabRects(int tabPlacement, int tabCount) {  
                ...
                for (i = 0; i < tabCount; i++) {  
                	//设置关闭按钮的大小，位置
                    closeRects[i].x = rects[i].x + rects[i].width - 15;  
                    closeRects[i].y = rects[i].y + 5;  
                    closeRects[i].width = 30;  
                    closeRects[i].height = 30;  
                }  
            }  
        }  
    }  
